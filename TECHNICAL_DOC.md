...
- **流式输出**:`QTextEdit` 逐段 append;Qt 6.4+ 可用 markdown 渲染
...
启动应用 → 显示最近会话列表 → 点击会话恢复 → 继续发送消息;同时支持新建、重命名和删除会话。删除会话只删除聊天记录,不删除论文库文件。
...
| **P2** | 对话面板:接入 LLM + SQLite 会话持久化,"问这篇论文"跑通(无工具) | 可跨天恢复的对话阅读器 | ★★ | 1~1.5 周 |
